...
\newpage
\noindent \textbf{Definition 2.2} Suppose $a_n$, $a_{n-1}$, $...$, $a_0$, $b_n$, $b_{n-1}$, $...$, $b_0$, and $c$ are propositions. Then, carry-out and sum outputs of an $N$-bit carry-ripple adder are defined as follows: 
\begin{enumerate}
	\item $ CRACO(a_n, ..., a_0, b_n, ..., b_0, c) :\equiv CO(a_n, b_n, CO(..., CO(a_0, b_0, c))...)$
	\item $ CRAS :\equiv S(a_n, b_n, CRACO(a_{n-1}, ..., a_0, b_{n-1}, ..., b_0, c)) $
\end{enumerate}
