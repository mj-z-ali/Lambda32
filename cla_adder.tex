...
\begin{align*}
	...
	&	\qquad \text{(I) Let $A = \lnot a_0, B = \lnot b_0, C = \lnot c$, then, we have:} \\
	&	\qquad \quad (i)  \: a_1 \land \lnot b_1 \land ((A \lor B) \land (A \lor C) \land (B \lor C))  \\
	&	\qquad \quad (ii)  \: a_1 \land \lnot b_1 \land (
		(A \land A \land B) \lor
		(A \land A \land C) \lor \\
		& \qquad \qquad \: (A \land C \land B) \lor
		(A \land C \land C) \lor
		(B \land A \land B) \lor \\
		& \qquad \qquad \: (B \land A \land C) \lor
		(B \land C \land B) \lor
		(B \land C \land C)) && \text{Distrib} \\
	&	\qquad \quad (iii)  \: a_1 \land \lnot b_1 \land ((A \land B) \lor(A \land C) \lor \\ 
	& \qquad \qquad \: (A \land B \land C) \lor (B \land C)) && \text{Idemp, Commut}  \\
	&	\qquad \quad (iv)  \: a_1 \land \lnot b_1 \land ((A \land B) \lor(A \land C) \lor (B \land C)) && \text{Absorp}  \\
	\intertext{The resulting three  disjunctions correspond to (1), (2) and (3) in $CLAS_1$}
\end{align*}
